...
if [ "$EUID" -ne 0 ]; then
  echo "Please run the script as root i.g: sudo ./ubuntu_setup.sh"
  exit
fi
...
# Install certbot for nginx (Optional)
if ! command -v certbot &>/dev/null; then
  sudo apt install certbot python3-certbot-nginx -y
fi

# sudo certbot --nginx -d codingveda.in -d www.codingveda.in -d jobs.codingveda.in -d scholarship.codingveda.in -d api.codingveda.in -d shop.codingveda.in -d staging.codingveda.in